    ...
    for i in 1:nalphas
        ...
        for j in 1:nbetas
            ...
            if haskey(bounds_dict,(a,b))
                ...
                axs[i,j].plot(plot_it, t_dual[n_init:n_end],   label="Dual", color="C1")
                #axs[i,j].plot(plot_it, t_inner[n_init:n_end],  label="Philpott", color="C2")
                axs[i,j].plot(plot_it, t_io[n_init:n_end],     label="Baucke", color="C3")
                axs[i,j].plot(plot_it, t_primal[n_init:n_end], label="Primal", color="C5")
            end
        end
        axs[1,nbetas].legend()
    end
